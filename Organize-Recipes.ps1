...
function Invoke-OrganizeRecipes {
    ...
    foreach ($file in $files) {
        ...
        if ($file.Extension.ToLower() -eq ".zip") {
            Process-ZipFile -ZipFile $file -DestPath $DestinationPath -Mode $Mode -Keywords $Keywords -LogPath $LogPath
            continue
        }
        ...
        if (Test-IsRecipe -Content $content -Filename $file.Name -Keywords $Keywords -LogPath $LogPath) {
            ...
            if ($Mode -eq "Test") {
                ...
                if ($Mode -eq "Copy") {
                    Write-Log -Message "Copying: $($file.Name) -> $targetFolder" -Level INFO -Color Green -LogPath $LogPath
                    Copy-Item -Path $file.FullName -Destination $targetFile -Force
                }
                elseif ($Mode -eq "Move") {
                    Write-Log -Message "Moving: $($file.Name) -> $targetFolder" -Level INFO -Color Green -LogPath $LogPath
...
function Get-DocxText {
    ...
    try {
        ...
        Write-Log -Message "Error reading .docx content for $Path : $_" -Level DEBUG -LogPath $LogPath
    }
    return ""
}

function Get-FileText {
    param ([System.IO.FileInfo]$File, [string]$LogPath)
    ...
    switch ($File.Extension.ToLower()) {
        ...
        ".docx" { 
            $content = Get-DocxText -Path $File.FullName -LogPath $LogPath
        }
        ".doc" {
...
function Test-IsRecipe {
    ...
    foreach ($keyword in $Keywords) {
        if ($Content -match "$keyword") {
            Write-Log -Message "Found keyword '$keyword' in content of $Filename" -Level DEBUG -LogPath $LogPath
            return $true
        }
    }
    ...
    if ($Filename -match "Recipe") {
        Write-Log -Message "Found 'Recipe' in filename of $Filename" -Level DEBUG -LogPath $LogPath
        return $true
    }
...
function Process-ZipFile {
    ...
    Write-Log -Message "Inspecting Zip Archive: $($ZipFile.Name)" -Level DEBUG -LogPath $LogPath
    try {
        Add-Type -AssemblyName System.IO.Compression.FileSystem
        ...
        foreach ($entry in $zip.Entries) {
            ...
            if ($entry.Name -match "Recipe") {
                $isRecipe = $true
                Write-Log -Message "Found 'Recipe' in zip entry filename: $($entry.Name)" -Level DEBUG -LogPath $LogPath
            }
            ...
            elseif ($entry.Name -match "\.(txt|xml|json|md)$") {
                ...
                foreach ($keyword in $Keywords) {
                    if ($text -match "$keyword") {
                        $isRecipe = $true
                        Write-Log -Message "Found keyword '$keyword' in zip entry content: $($entry.Name)" -Level DEBUG -LogPath $LogPath
                        break
                    }
                }
            }
            ...
            if ($isRecipe) {
                ...
                if ($Mode -eq "Test") {
                    Write-Log -Message $actionMsg -Level INFO -Color Green -LogPath $LogPath
                    Write-Log -Message "  -> Would extract to: $targetFile" -Level INFO -Color Gray -LogPath $LogPath
                }
                else {
                    ...
                    Write-Log -Message "Extracting: $($entry.Name) -> $targetFolder" -Level INFO -Color Green -LogPath $LogPath
